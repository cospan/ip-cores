...
from axi_i2s_driver import axi_i2sDriver
...
def setup_dut(dut):
    ...
    cocotb.start_soon(Clock(dut.axis_audio_clk, CLK_AXIS_PERIOD).start())
...
async def reset_audio_dut(dut):
    dut.axis_audio_rst.value = 1
    await Timer(CLK_AXIS_PERIOD * 10)
    dut.axis_audio_rst.value = 0
    await Timer(CLK_AXIS_PERIOD * 10)
...
async def test_read_version(dut):
    ...
    dut.test_id.value = 0
    setup_dut(dut)
    driver      = axi_i2sDriver(dut, dut.clk, dut.rst, CLK_PERIOD, name="aximl", debug=False)
    axis_sink   = AXISSink  (dut, "axis_audio", dut.axis_audio_clk, dut.axis_audio_rst)
    await reset_dut(dut)
    await reset_audio_dut(dut)
...
async def test_write_control(dut):
    ...
    await driver.select_triangle_or_sine_wave(True)
    await driver.enable_audio_fifo(True)
    await driver.enable_wave(True)
    #dut_control = dut.dut.r_control.value
    #dut._log.debug ("Control: 0x%08X" % dut.dut.r_control.value)
    await Timer(CLK_PERIOD * 500)
    dut._log.debug("Done")
